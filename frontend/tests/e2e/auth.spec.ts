...
test.describe("auth", () => {
  test("register: 201 + user + sets two cookies", async ({ request }) => {
    ...
    const { status, body, headers } = await postJson(request, "/auth/register", {
      email,
  ...
  test("register: duplicate email → 409", async ({ request }) => {
    ...
    await postJson(request, "/auth/register", { email, password: "password123" });
  ...
  test("register: short password → 400", async ({ request }) => {
    const { status } = await postJson(request, "/auth/register", {
  ...
  test("login: valid credentials → 200 + user, no password_hash", async ({ request }) => {
    ...
    const { status, body } = await postJson(request, "/auth/login", {
      email,
      password: "password123",
    });
  ...
  test("refresh: valid refresh cookie → 200 + rotated", async ({ request }) => {
    ...
    const { status, body } = await postJson(request, "/auth/refresh", {});
    expect(status).toBe(200);
    expect((body as { data: { user: { email: string } } }).data.user.email).toBe(email);
  });
  ...
  test("refresh: after logout → 401", async ({ request }) => {
    ...
    await postJson(request, "/auth/register", { email, password: "password123" });
    await postJson(request, "/auth/logout", {});

    const { status } = await postJson(request, "/auth/refresh", {});
    expect(status).toBe(401);
  });
  ...
  test("logout → 204, then me → 401", async ({ request }) => {
    ...
    const { status } = await postJson(request, "/auth/logout", {});
